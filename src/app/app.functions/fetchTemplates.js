...
async function updateHubDbTable(hubspotClient, userID, newRefreshToken, templatesJsonUrl) {
    ...
    // Check last template sync date
    const lastTemplateSyncDate = new Date(existingUserRow.values.lastTemplateSyncDate);
    const now = new Date();
    const hoursSinceLastSync = (now - lastTemplateSyncDate) / 1000 / 60 / 60;

    if (hoursSinceLastSync > 24 || !existingUserRow.values.templatesfeed) {
        console.log("Re-fetching templates since it's been more than 24 hours or no template link exists.");

        // Re-fetch templates
        const templatesData = await getTemplates(existingUserRow.values.refreshToken, existingUserRow.values.marqUserID);
        ...
        // Update the user's row with the new values and last sync date
        const rowValues = {
            refreshToken: templatesData.newRefreshToken,
            templatesfeed: templatesData.templatesJsonUrl,
            lastTemplateSyncDate: now.toISOString() // Update sync date
        };
        ...
        console.log("Template link is up-to-date; no need to re-fetch.");
    }
...
exports.main = async (context) => {
    ...
    try {
        await updateHubDbTable(hubspotClient, userID, refreshToken, null); // Pass null for templatesJsonUrl to force re-fetch if necessary
    } catch (error) {
